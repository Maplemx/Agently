...
class OpenAIAssistant(ComponentABC):
    ...
    def _create_client(self):
        client_params = {}
        base_url = self.agent.request.plugin_manager.get_settings("model_settings.url")
        if base_url:
            client_params.update({ "base_url": base_url })
        proxy = self.agent.request.plugin_manager.get_settings("proxy")
        if proxy:
            client_params.update({ "http_client": httpx.Client( proxies = proxy ) })
        api_key = self.agent.request.plugin_manager.get_settings("model_settings.auth.api_key")
        if api_key:
    ...
    def update(self, *, force_to_create: bool=False):
        ...
        model = self.agent.request.plugin_manager.get_settings(
            "model_settings.options.model",
            "gpt-3.5-turbo-1106",
        )
        tools = self.assistant_runtime_ctx.get("tools", [])
        
        assistant_id = self.agent.request.plugin_manager.get_settings("model_settings.assistant_id")
        # update
