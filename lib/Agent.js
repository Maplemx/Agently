...
class AgentSession {
    ...
    addStreamingHandler (streamingHandler) {
        if (streamingHandler) this.streamingHandlers.push(streamingHandler)
        return this
    }
...
class Agent {
    ...
    setSkills (skillNames) {
        if (!(skillNames instanceof Array)) skillNames = [skillNames]
        for (let i = 0; i < skillNames.length; i++) {
            if (this.agentSettings.skills.indexOf(skillNames[i]) === -1) this.agentSettings.skills.push(skillNames[i])
        }
        return this
    }*/
