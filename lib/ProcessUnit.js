...
class ProcessRequest {
    ...
    async start(requestType) {
        ...
        if (this.getAgentSettings().useSkills && this.prefix.generateSkillTips) {
            skillJudgeMessages = [ ...requestMessages ]
        ...
        if (this.getAgentSettings().useSkills && this.prefix.generateSkillJudgePrompt) {
            ...
            try {
                ...
                if (parsedJudgeResponse.length > 0) {
                    ...
                    for (let i = 0; i < parsedJudgeResponse.length; i++) {
                        skillResponseMessage.content += `-[${ parsedJudgeResponse[i].skillName }]: ${ JSON.stringify(skillList[parsedJudgeResponse[i].skillName].handler(parsedJudgeResponse[i].data)) }\n\n`
                    }
